// COMS20001 - Cellular Automaton Farm - Multi-Distributor Geometric Farming Hybrid (Bytes)
...
#define MAX_ROUNDS 1  // Maxiumum number of rounds to be processed (uncomment relevant if statement in Master Distributor).
...
char outfname[] = "64x64out(4w-4s-1r).pgm";  // Output image path
...
void printStatusReport(double totalTime, int rounds, int liveCells, int final) {
    ...
    printf("Rounds Processed: %d\n"
           ...
           "Number of workers: %d\n"
           "----------------------------------\n\n",
           rounds, liveCells, IMHT*IMWD, totalTime, totalTime/rounds, WORKERS);
}
...
void masterDistributor(chanend c_fromButtons, chanend c_toLEDs, chanend c_in, chanend c_out, chanend fromAcc, chanend c_toSlave[2]) {
    ...
    uchar edges[4][IMHT];     // Overlapping edges to be passed between the slave distributors.
    uchar pixel;              // A single pixel being sent to a slave distributor.
    ...
    // Send edges to the slave distributors for the first round.
    for (int y = 0; y < IMHT; y++) {
        ...
        select {
            ...
            case c_fromButtons :> buttonPressed:
                if (buttonPressed == SW2) {
                    ...
                    // Send signal to slave distributors to stop and send their image.
                    c_toSlave[0] <: STOP;
                    c_toSlave[1] <: STOP;
                    ...
                    printStatusReport(totalTime, rounds, liveCells[0]+liveCells[1], 1);
            ...
            case fromAcc :> tilted:
                ...
                // Send signal to slave distributors that processing is paused.
                c_toSlave[0] <: PAUSE;
                c_toSlave[1] <: PAUSE;
                ...
                // Print paused state status report.
                printStatusReport(totalTime, rounds, liveCells[0]+liveCells[1], 0);

                // Board horizontal.
                fromAcc :> tilted;
                printf("Board horizontal\n");

                // Send signal to slave distributors to continue.
            ...
            default:
                if (rounds < MAX_ROUNDS) {  // UNCOMMENT THIS WHEN PROCESSING A SPECIFIED NUMBER OF ROUNDS.
                    ...
                    // Send signal to slave distributors to process another round.
                    c_toSlave[0] <: CONTINUE;
                    c_toSlave[1] <: CONTINUE;

                    // Receive new edges for each slave distributor.
                    for (int y = 0; y < IMHT; y++) {
                    ...
                    // Pass new edges received from one slave distributor to the other.
                    for (int y = 0; y < IMHT; y++) {
                        c_toSlave[0] <: edges[3][y];
                        c_toSlave[0] <: edges[2][y];
                        c_toSlave[1] <: edges[1][y];
...
void slaveDistributor(chanend c_fromMaster, chanend c_toWorker[n], unsigned n, int distNum) {
    ...
    int distWD = IMWD/2 + 2;           // Width of a slave distributor image segment.
    ...
    // Processing the rounds.
    int running = 1;  // Whether to keep processing rounds.
    while (running) {

        // Receive signal from the MasterDistributor advising what to do next.
        int signal;
        c_fromMaster :> signal;
        ...
        if (signal == CONTINUE) {
            ...
            int workerSegs[WORKERS/2];  // Number of segments contributed by each worker.
            ...
            */
            ...
            /* UNCOMMENT WHEN SHOWING CORE PRIORITISATION.
            for (int i = 0; i < WORKERS/2; i++) {
                printf("[SLAVE %D] Segments contributed by worker %d: %d\n", distNum, i, workerSegs[i]);
            }
            */
